            ...
            <div id="sLocation" class="collapse in">
              <ul class="list-group" id="location_filter">
                ...
                if(count($location)>0)
                {
                  ...
                  {?>
                  <li class="checkbox"><input onchange="applyFilter()" value="<?php echo $row['id']; ?>" <?php echo ($searchLocation == $row['id']) ? 'checked': ''; ?> class="check_location" type="checkbox" id="" /> <label for=""><?php echo $row['name']; ?></label></li>
            ...
            <div id="sIndustry" class="collapse in">
              ...
              <?php
                if(count($industry)>0)
                {
                  ...
                  {?>
                  <li class="checkbox"><input class="check_industry" value="<?php echo $row['id']; ?>" type="checkbox" onchange="applyFilter()" id="<?php echo $row['id']; ?>" /> <label ><?php echo $row['name']; ?></label></li>
...
function sortBy(val)
{
  ...
  function onLoad()
  {
    ...
    $.ajax({
      ...
      data:{'group_no':group_load,'get_total_group':1,'orderby':orderby,'skills':skills,'location':location,'industry':industry,'experience':experience,'last_days':last_days,'salary':salary},
      type:'POST',
      dataType:'json',
  ...
  function lazzyLoad()
  {
    ...
    $.ajax({
      url:base_url+"job/job_ajax",
      data:{'group_no':group_load,'orderby':orderby,'skills':skills,'location':location,'industry':industry,'experience':experience,'last_days':last_days,'salary':salary},
      type:'POST',
      dataType:'json',
      success:function(data){
